...
export var zutil = {
  ...
  getChildenByType: function (root_dom, type) {
    var self = this;
    var typeParent;
    var arr = [];
  ...
  getAllElements: function (root_dom) {
    ...
    for (var i = 0; i < root_dom.numChildren; i++) {
      let child = root_dom.getChildAt(i)
      arr.push(child);
      arr = arr.concat(self.getAllElements(child));
    }
    return arr;
  },
  // 获取所有下级node
  ...
  queryElements: function (root_dom, queryString) {
    ...
    for (var i = 0; i < allElements.length; i++) {
      if (self.queryCheck(root_dom, allElements[i], queryArr)) {
  ...
  queryCheck(root_dom, item_dom, queryArr) {
    var self = this;
    ...
    if (self.closestCheck(root_dom, item_dom, queryArr)) {
      return true;
    }
  },
  closestCheck: function (root_dom, item_dom, queryArr) {
    var self = this;
    var lastQueryStr = queryArr[queryArr.length - 1];
    var funcSelf = self.closestCheck.bind(self);
    ...
    return self.closestCheck(root_dom, parent_dom, queryArr);
  },
  ...
  // log
  createLog: function (log_type) {
    var self = zutil;
    var empty_fn = function () {};

    var type = log_type || self.debugType2();
  ...
  ellipsisStr: function (text, max_len, append_str) {
    /**非NaN数字转化为字符串*/
  ...
  covertDom: function (sprite, new_class) {
    var new_sprite = new new_class();
   ...
   * num显示的位数, 秒,分,时, 2: 分秒
   */
  formatTime: function (time, num) {
    var hours = Math.floor(time / 3600);
    var minuts = Math.floor((time % 3600) / 60);
    var seconds = time % 60;

    var time_arr = [hours, minuts, seconds];
    var time_str = '';
  ...
  addPressEvent: function (sprite) {
    /**按下, 开始记录*/
    sprite.on(Laya.Event.MOUSE_DOWN, sprite, function (event) {
      sprite.cus_event = {
        status: 'MOUSE_DOWN',
      ...
      Laya.timer.once(1000, sprite, function () {
        sprite.event('longPress');
      });
    });
    /**取消*/
    sprite.on(Laya.Event.MOUSE_MOVE, sprite, function (event) {
      if (!sprite.cus_event) {
        return;
      }
      sprite.cus_event = {
    ...
    sprite.on(Laya.Event.MOUSE_UP, sprite, function (event) {
      if (!sprite.cus_event) {
        return;
      }
