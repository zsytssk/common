...
filePath = (new File($.fileName)).parent + '/shoal1.data.json';
...
function getAlllayersInfo(layerSet, fishType) {
  var fish = [];
  ...
  for (var l_len = layers.length, j = l_len-1; j>=0; j--) {
    ...
    var fishInfo = getLayerInfo(layer, fishType, isSpecial);
    fish.push(fishInfo);
  }
  ...
  for (var l_len = layerSets.length, j = l_len-1; j>=0; j--) {
    ...
    var name  = parseInt(layerSet.name);
    var fishType = name ? name : fishType;
    var layerSet_fish = getAlllayersInfo(layerSet, fishType);
...
function getLayerInfo(layer, fishType, isSpecial) {
